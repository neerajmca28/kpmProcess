...
class SupportModel extends CI_Model {
  ...
  public function update_kpm_services($data){
      ...
      public function show_ServicesTable($data){
        //print_r($data);die();
       //echo $data['datetoupdate'];die;
        $CI = &get_instance();
        $matrix_db = $this->load->database('matrix_db', TRUE);
        
       $dateupdate=explode('-',$data['id']);
       //print_r($data); die;
        //$date=date('Y-m-d',$data['datetoupdate']);
       //echo "select kpm_tripdata.warehouse, kpm_tripdata.tripplan,kpm_tripdata.rate, services.veh_reg,services.thana_name,services.veh_destination,services.veh_driver_name,veh_departuredate,driver_contact_no,veh_phone_number,card_number,veh_advance from services inner join kpm_tripdata on services.id=kpm_tripdata.vehicle_number where services.id='".$data['recordId']."' and trip_start_date='".$data['datetoupdate']."'";die;
        $userIdQuery= "select kpm_tripdata.from_location,kpm_tripdata.weight,kpm_tripdata.seal_no,kpm_tripdata.warehouse, kpm_tripdata.tripplan,kpm_tripdata.rate, services.veh_reg,services.thana_name,services.veh_destination,services.veh_driver_name,veh_departuredate,driver_contact_no,veh_phone_number,card_number,veh_advance from services inner join kpm_tripdata on services.id=kpm_tripdata.vehicle_number where services.id='".$data['recordId']."' and trip_start_date='".$data['datetoupdate']."'";
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    //$userId='7781';
                    $userId = $this->session->userdata['user_id_live'];
                    $groupId = $this->session->userdata['group_id'];
                   $installerId = $this->session->userdata['installerId'];
                     $sql_req_dev="insert into matrix.requested_device(group_id,imei,mobile_no,device_id,veh_reg,sales_user_id,device_status) values('".$groupId."','".$dev_imei."','".$phone_no."','".$device_id."','".$vehicle_no."','','2')";
                    ...
                    if($userId!=1998)
                     {
                        $sql6="insert into matrix.group_services(sys_service_id,sys_group_id,sys_added_date) values('".$serviceid."','".$groupId."','".$date."')";
                        //echo $sql6;die;
                        ...
                        $sql7="insert into matrix.tbl_history_devices(`sys_group_id`,`sys_service_id`) values('".$groupId."','".$serviceid."')";
                         //echo  $sql7; die;
                      ...
                      $query_lat_tele=$matrix_db->query($sql9);
                        ...
                        // echo "INSERT INTO `latest_telemetry_cellid` ( `sys_service_id`, `sys_proc_time`, `message_type`, `host_name`, `mnc`, `mcc`, `lac`, `cellid`, `gps_time`, `imei`, `rawdata`) VALUES ( '".$serviceid."', now(), 1, 'WIN-1V4ASLD1M15', 10, 404, 516, 40712, 'now()', '".$dev_imei."', '')";
                        


                        $sqlcell= "INSERT INTO `latest_telemetry_cellid` ( `sys_service_id`, `sys_proc_time`, `message_type`, `host_name`, `mnc`, `mcc`, `lac`, `cellid`, `gps_time`, `imei`, `rawdata`) VALUES ( '".$serviceid."', now(), 1, 'WIN-1V4ASLD1M15', 10, 404, 516, 40712, 'now()', '".$dev_imei."', '')";
                         $query_cellId=$matrix_db->query($sqlcell);
            ...
            //  if('".$data['source']."')
                ...
                $sqltrip="INSERT INTO `kpm_tripdata` (`vehicle_number`, `veh_reg`, `to_location`, `Topoi_lat`, `Topoi_long`, `To_exit_Datetime`, `To_entry_Datetime`, `trip_start_date`, `from_location`, `frompoi_lat`, `frompoi_long`, `From_exit_Datetime`, `From_entry_Datetime`, `total_Km`, `from_poiid`, `to_poiid`, `loaded_empty`, `driver_code`, `contact_number`, `client_name`, `client_company_name`, `email_id`, `LR_Number`, `driver_name`, `driver_number`, `status`, `add_type`,warehouse,tripplan,rate,seal_no,weight,actual_start_time,sys_group_id,device_imei) 
                VALUES ('".$serviceid."', '".$data['truck_no']."', '".$data['destination']."', '00.00000000', '00.00000000', NULL, NULL, '".$data['date']."', '".$data['source']."', '00.00000000', '00.00000000', NULL, NULL, '0', '0', '0', '1', '0', '".$data['transport_mob_no']."', '".$data['transport_name']."', '".$data['customer_name']."', '', '".$data['lr_number']."', NULL, '".$data['driver_mobile_no']."', 1, 'new', '".$data['warehouse']."', '".$data['trip']."', '".$data['rate']."', '".$data['sealno']."', '".$data['weight']."', '".$data['actual_start_time']."','".$groupId."','".$data['device_imei']."')";
                //echo $sqltrip; die;
    ...
    public function changeVehicle($data)
    {
            ...
            $matrix_db->trans_begin();
             ...
             // print_r($query_arr); die;
              ...
              $changedby= "Changed by ".$data['username']." office";
              $ins_change_vehicle_history="INSERT INTO matrix.change_vehicle_history(service_id,group_id,old_veh_number,new_veh_number,reason,`date`,updated_by) values('".$data['device_no_old']."','".$data['groupId']."','" . $old_veh_reg."','".$data['device_no_new']."','".$changedby."','".date('Y-m-d H:i:s')."','".$data['username']."')";
