    ...
    public function new_device_additionclose($data,$data_lrnumber)
    {
       ...
       // echo '<pre>'; print_r($data); die;
        ...
        //echo '<pre>'; print_r($query->result_array()); die;
         ...
         // $xml = simplexml_load_file('http://203.115.101.54/inventory/getimeisim.php?ItgcID='.$device_id);
         //  $i=0;
         //  foreach ($xml as $k)
         //  {
         //     $arr[$i]=$k;
                    ...
                    $SELECT_services='SELECT sys_device_id FROM matrix.services where sys_device_id=? limit 1';
                     ...
                     else
                     {
                      ...
                      if($flag!=1)
                      {
                        ...
                        {
                             $SELECT5="select name from matrix.group where id='".$groupId."'";
                             //echo $SELECT5; die;
                             $query5=$matrix_db->query($SELECT5, array($groupId));
...
*/
 ...
 $sqltrip="INSERT INTO `kpm_tripdata` (`vehicle_number`, `veh_reg`, `to_location`, `Topoi_lat`, `Topoi_long`, `To_exit_Datetime`, `To_entry_Datetime`, `trip_start_date`, `from_location`, `frompoi_lat`, `frompoi_long`, `From_exit_Datetime`, `From_entry_Datetime`, `total_Km`, `from_poiid`, `to_poiid`, `loaded_empty`, `driver_code`, `contact_number`, `client_name`, `client_company_name`, `email_id`, `LR_Number`, `driver_name`, `driver_number`, `status`,  `add_type`,warehouse,tripplan,rate,seal_no,weight,actual_start_time,sys_group_id,device_imei) 
                VALUES ('".$serviceid."', '".$data['truck_no']."', '".$data['destination']."', '00.00000000', '00.00000000', NULL, NULL, '".$data['date']."', '".$data['source']."', '00.00000000', '00.00000000', NULL, NULL, '0', '0', '0', '1', '0', '".$data['transport_mob_no']."', '".$data['transport_name']."', '".$data['customer_name']."', '', '".$data['lr_number']."', NULL, '".$data['driver_mobile_no']."', 1,  'new', '".$data['warehouse']."', '".$data['trip']."', '".$data['rate']."', '".$data['sealno']."', '".$data['weight']."', '".$data['actual_start_time']."','".$groupId."','".$dev_imei."')";
